...
namespace ImageProcessorCore.Formats
{
    ...
    public class JpegEncoder2 : IImageEncoder
    {
            ...
            where TColor : struct, IPackedPixel, IEquatable<TColor>
        {
            ...
            using (EndianBinaryWriter writer = new EndianBinaryWriter(Endianness.BigEndian, stream))
            {
        ...
        private void WriteApplicationHeader<TColor>(Image<TColor> image, EndianBinaryWriter writer)
            where TColor : struct, IPackedPixel, IEquatable<TColor>
        {
        ...
        /// <param name="image">The image to encode from.</param>
        /// <param name="writer">The writer to write to the stream.</param>
        private void WriteStartOfFrame<TColor>(ImageBase<TColor> image, EndianBinaryWriter writer)
            where TColor : struct, IPackedPixel, IEquatable<TColor>
        {
        ...
        /// <param name="image">The image to encode from.</param>
        /// <param name="writer">The writer to write to the stream.</param>
        private void WriteStartOfScan<TColor>(ImageBase<TColor> image, EndianBinaryWriter writer)
            where TColor : struct, IPackedPixel, IEquatable<TColor>
        {
            // Marker
        ...
        private void RgbToYcbCr<TColor>(ImageBase<TColor> image, float[] yComponant, float[] cbComponant, float[] crComponant, int x, int y)
            where TColor : struct, IPackedPixel, IEquatable<TColor>
        {
            ...
            using (PixelAccessor<TColor> pixels = image.Lock())
            {
                for (int a = 0; a < 8; a++)
                {
                    ...
                    for (int b = 0; b < 8; b++)
                    {
                        ...
                        TColor c = pixels[px, py];

                        var scaleFunc = PackedPixelConverterHelper.ComputeScaleFunction<TColor, Color>(null);
                        Vector4 v = c.ToVector4();
                        v = scaleFunc(v);

                        // Convert returned bytes into the YCbCr color space. Assume RGBA
                        yComponant[index] = (0.299F * v.X) + (0.587F * v.Y) + (0.114F * v.Z);
                        cbComponant[index] = 128 + ((-0.168736F * v.X) - (0.331264F * v.Y) + (0.5F * v.Z));
                        crComponant[index] = 128 + ((0.5F * v.X) - (0.418688F * v.Y) - (0.081312F * v.Z));
